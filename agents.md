...
### 24. Manual Schema Drift Reconciliation & Introspective Code Alignment
- **Manual Mutation Catching**: When changes are made manually inside the Supabase visual Table Editor UI (adding columns, altering data constraints, changing row keys), the system must explicitly perform an introspection phase before generating code.
- **Strict Client-Side Synchronicity**: Every manual change in the database layout must immediately trigger a localized update pass. You MUST forcefully update type schemas inside `types.ts`, mapping components, and database fetch/upsert parameters inside `src/lib/db.ts` to reflect the new columns and data types down to the exact string case.
- **Cache Invalidation Enforcement**: Whenever database columns or relational constraints are modified via the UI, a SQL schema cache notification (`NOTIFY pgrst, 'reload schema';`) must be run to ensure Supabase's PostgREST API immediately recognizes the changes without dropping connection states.